...
SAVE_PATH = os.path.abspath('/media/jeeheon/SSD/ToothFairy_Dataset/sparse2dense')
...
for data_type in save_types:
    ...
    for idx, file_name in enumerate(json_data[data_type]):
        ...
        print('np_sparse.shape : ', np_sparse.shape)
        print('np_gt.shape : ', np_gt.shape)

        nii_sparse = nib.Nifti1Image(np_sparse, affine=np.eye(4))
        nii_gt = nib.Nifti1Image(np_gt, affine=np.eye(4))

        nib.save(nii_sparse, save_each_dir + '/gt_sparse.nii.gz')
